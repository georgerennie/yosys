...
// Types used for storing -set/-prove etc values. Parameterised to support
// both versions storing strings and versions storing sigspecs
template <class T> using Sigs = pool<T>;
template <class T> using SigVals = std::vector<std::pair<T, T>>;
 // As above but referring to specific steps
template <class T> using StepSigs = dict<int, Sigs<T>>;
template <class T> using StepSigVals = dict<int, SigVals<T>>;

struct SatConfig
{
	int  num_solutions = 1;     // Number of solutions to enumerate. N for -max <N> or -1 for -all
	static constexpr int AllSolutions = -1;
	bool enable_undef  = false; // Model undef values
	bool max_undef     = false; // Maximimize undef bits in solutions
	Sigs<SigSpec> shows;        // Signals to show

	bool ignore_div_by_zero   = false; // Ignore solutions that have division by zero
	bool ignore_unknown_cells = false; // Ignore cells without a SAT model
	bool set_def_formal       = false; // Add set-def constraints to $anyinit, $anyconst, $anyseq

	// For the following, timestep -1 represents constraints that always apply
	static constexpr int Always = -1;
	StepSigVals<SigSpec> sets;           // Signals set to values at times
	StepSigs<SigSpec>    unsets;         // Signals unset (overriding sets) at times
	StepSigs<SigSpec>    sets_def;       // Signals constrained to be defined at times
	StepSigs<SigSpec>    sets_any_undef; // Signals with at least one bit constrained undef at times
	StepSigs<SigSpec>    sets_all_undef; // Signals with all bits constrained to be undef at times
	SigVals<SigSpec>     sets_init;      // Signals set to values in initial state

	bool set_init_undef = false; // Set all initial states without a sets_init to undef
	bool set_init_def   = false; // Do not allow undef values in initial state
	bool set_init_zero  = false; // Set all initial states without a sets_init to 0
	bool set_assumes    = false; // Only show solutions that don't violate assumptions

	// File to dump the model/trace to. Writes the last trace found in the
	// case that multiple are found
	std::string vcd_file_name;
	std::string json_file_name;
	std::string cnf_file_name;

	// Enabled with -tempinduct, in this mode bound is increased in steps of
	// stepsize, otherwise a single monolithic query of maxsteps is used
	bool incremental     = false;
	bool basecase        = true;  // Run the basecase
	bool inductcase      = false; // Try to find an inductive proof with the inductive case
	bool tempinduct_def  = false; // Assume all registers start with defined values in inductive step
	int  tempinduct_skip = 0;     // Skip the first N steps of inductive case

	struct ProofGoal;
	std::vector<ProofGoal> proof_goals; // Targets to prove

	int prove_skip = 0; // Don't enforce prove condition for first N steps
	int maxsteps   = 0; // Max timesteps to unroll, set by -maxsteps or -seq
	int initsteps  = 0; // Initial length for induction
	int stepsize   = 1; // Amount to increase the size of induction proof

	// If verify is specified, return an error if a proof fails, if falsify is
	// specified return an error if a proof succeeds
	enum class RequireResult {
		Any,
		Verify,
		Falsify
	} require_result = RequireResult::Any;

	bool error_on_timeout = false; // If any call times out, return an error
	int timeout           = 0; // Max seconds per SAT call, 0 for no timeout
...
struct ProofGoal {
	struct Eq {
		...
		Eq(SigSpec lhs, SigSpec rhs, bool allow_x_lhs = false) :
			lhs{lhs}, rhs{rhs}, allow_x_lhs{allow_x_lhs} {}

		int importProofBit(SatGen& satgen, int timestep = -1) const {
			if (!allow_x_lhs)
				return satgen.signals_eq(lhs, rhs, timestep);

			log_assert(GetSize(lhs) == GetSize(rhs));
			const auto value_lhs = satgen.importDefSigSpec(lhs, timestep);
			const auto value_rhs = satgen.importDefSigSpec(rhs, timestep);

			const auto undef_lhs = satgen.importUndefSigSpec(lhs, timestep);
			const auto undef_rhs = satgen.importUndefSigSpec(rhs, timestep);

			auto& ez = satgen.ez;
			const auto def_and_eq = ez->vec_and(ez->vec_not(undef_rhs), ez->vec_iff(value_lhs, value_rhs));
			return ez->vec_reduce_and(ez->vec_or(undef_lhs, def_and_eq));
		}
	};

	// Eq is as generated by -prove and -prove-x and RTLIL::Cell* stores $assert cells
	std::variant<Eq, RTLIL::Cell*> target;

	ProofGoal(const Eq& eq) : target{eq} {}
	ProofGoal(RTLIL::Cell* cell) : target{cell} {}
	static ProofGoal prove_eq(std::pair<SigSpec, SigSpec> p) { return { Eq{p.first, p.second, false} }; }
	static ProofGoal prove_eq_x(std::pair<SigSpec, SigSpec> p) { return { Eq{p.first, p.second, true} }; }

	// To allow functional definitions for std::visit
	template<class... Ts> struct overload : Ts... { using Ts::operator()...; };
	template<class... Ts> overload(Ts...) -> overload<Ts...>;

	int importGoal(SatGen& satgen, int timestep = -1) {
		return std::visit(overload{
			[&](const Eq& eq) { return eq.importProofBit(satgen, timestep); },
			[&](RTLIL::Cell* cell) { return satgen.importPropertyCell(cell, timestep); },
		}, target);
	}
};
...
struct SatHelper
{
	...
	SatHelper(SatConfig& config, RTLIL::Design *design, RTLIL::Module *module) :
		config(config), design(design), module(module), sigmap(module), ct(design), satgen(ez.get(), &sigmap)
	{
		satgen.model_undef = config.enable_undef;
		satgen.def_formal = config.set_def_formal;
		satgen.ignore_div_by_zero = config.ignore_div_by_zero;
		max_timestep = -1;
		timeout = 0;
		gotTimeout = false;
	}
	...
	void setup(int timestep = SatConfig::Always, bool initstate = false)
	{
		if (timestep > 0)
		...
		// These are the timesteps to import constraints from, those that are
		// always valid and those that are only valid in this step. If the current
		// step is SatConfig::Always and thus we are solving a SAT problem, we only
		// need to add constraints once
		std::vector<int> constr_timesteps = {SatConfig::Always};
		if (timestep != SatConfig::Always)
			constr_timesteps.emplace_back(timestep);
		...
		// Collect together set signals
		for (const auto constr_step : constr_timesteps) {
			for (const auto &[lhs, rhs] : config.sets[constr_step]) {
				show_signal_pool.add(sigmap(lhs));
				show_signal_pool.add(sigmap(rhs));
				log("Import set-constraint%s: %s = %s\n", constr_step == SatConfig::Always ? "" : " for this timestep",
		...
		// Remove unset signals
		for (const auto constr_step : constr_timesteps) {
			for (const auto &sig : config.unsets[constr_step]) {
				show_signal_pool.add(sigmap(sig));
				log("Import unset-constraint for this timestep: %s\n", log_signal(sig));
				big_lhs.remove2(sig, &big_rhs);
			}
		}
		...
		pool<RTLIL::SigSpec> sets_def, sets_any_undef, sets_all_undef;

		// Add a set def or any/all undef constraint. If a signal has appeared
		// in other sets already it is overwritten, so the order that signals are
		// added to this matters
		const auto add_sets_def = [&](const auto& sigs, auto& set) {
			for (const auto &sig : sigs) {
				// Overwrite by removing it from all the sets
				sets_def.erase(sig);
				sets_any_undef.erase(sig);
				sets_all_undef.erase(sig);

				set.insert(sig);
			}
		};

		// Collect together def constraints, first applying global ones than
		// timestep specific ones that overwrite the global constraints
		for (const auto constr_step : constr_timesteps) {
			add_sets_def(config.sets_def[constr_step], sets_def);
			add_sets_def(config.sets_any_undef[constr_step], sets_any_undef);
			add_sets_def(config.sets_all_undef[constr_step], sets_all_undef);
		}

		// Import these def constraints to the SAT problem
		for (const auto &sig : sets_def) {
			log("Import def constraint for this timestep: %s\n", log_signal(sig));
			ez->assume(ez->NOT(ez->expression(ezSAT::OpOr, satgen.importUndefSigSpec(sig, timestep))));
		}

		for (const auto &sig : sets_any_undef) {
			log("Import any_undef constraint for this timestep: %s\n", log_signal(sig));
			ez->assume(ez->expression(ezSAT::OpOr, satgen.importUndefSigSpec(sig, timestep)));
		}

		for (const auto &sig : sets_all_undef) {
			log("Import all_undef constraint for this timestep: %s\n", log_signal(sig));
			ez->assume(ez->expression(ezSAT::OpAnd, satgen.importUndefSigSpec(sig, timestep)));
		}
		...
		for (auto cell : module->cells())
			if (design->selected(module, cell)) {
				if (!satgen.importCell(cell, timestep)) {
					if (config.ignore_unknown_cells)
				...
				if (config.set_assumes && cell->type == ID($assume)) {
					log("Import constraint from assume cell: %s when %s.\n", log_signal(cell->getPort(ID::A)), log_signal(cell->getPort(ID::EN)));
					ez->assume(satgen.importPropertyCell(cell, timestep));
				}
		...
		if (initstate)
		{
			...
			if (config.set_init_undef && satgen.def_formal)
				for (auto cell : module->cells())
			...
			for (const auto& [lhs, rhs] : config.sets_init) {
				show_signal_pool.add(sigmap(lhs));
				show_signal_pool.add(sigmap(rhs));
	...
	int setup_proof(int timestep = SatConfig::Always)
	{
		RTLIL::SigSpec big_lhs, big_rhs;
	...
	{
		log_assert(config.enable_undef);
	...
	void generate_model()
	{
		...
		{
			for (const auto &sig : config.shows) {
	...
	void print_model()
	{
		...
		for (auto &info : modelInfo)
		{
			...
			for (int i = 0; i < info.width; i++) {
				...
				if (config.enable_undef && modelValues.at(modelExpressions.size()/2 + info.offset + i))
...
{
	log("\n");
	log(R"(   ______                   ___       ___       _ _            _ _ )" "\n");
	log(R"(  (_____ \                 / __)     / __)     (_) |          | | |)" "\n");
	log(R"(   _____) )___ ___   ___ _| |__    _| |__ _____ _| | _____  __| | |)" "\n");
	log(R"(  |  ____/ ___) _ \ / _ (_   __)  (_   __|____ | | || ___ |/ _  |_|)" "\n");
	log(R"(  | |   | |  | |_| | |_| || |       | |  / ___ | | || ____( (_| |_ )" "\n");
	log(R"(  |_|   |_|   \___/ \___/ |_|       |_|  \_____|_|\_)_____)\____|_|)" "\n");
	log("\n");
}
...
{
	log("\n");
	log(R"(        _____  _  _      _____ ____  _     _____ )" "\n");
	log(R"(       /__ __\/ \/ \__/|/  __//  _ \/ \ /\/__ __\)" "\n");
	log(R"(         / \  | || |\/|||  \  | / \|| | ||  / \  )" "\n");
	log(R"(         | |  | || |  |||  /_ | \_/|| \_/|  | |  )" "\n");
	log(R"(         \_/  \_/\_/  \|\____/\____/\____/  \_/  )" "\n");
	log("\n");
}
...
void print_qed()
{
	...
	log(R"(                | $$/$$ $$    | $$          | $$  | $$   )" "\n");
	log(R"(                |  $$$$$$/ /$$| $$$$$$$$ /$$| $$$$$$$//$$)" "\n");
	log(R"(                 \____ $$$|__/|________/|__/|_______/|__/)" "\n");
	log(R"(                       \__/                              )" "\n");
	log("\n");
}
...
struct SatPass : public Pass {
	...
	void execute2(std::vector<std::string> args, RTLIL::Design *design)
	{
		log_header(design, "Executing SAT pass (solving SAT problems in the circuit).\n");

		SatConfig config;
		enum class ProveMode { Comb, Seq, TempInduct } mode = ProveMode::Comb;
		enum class TempInductCases { Both, BaseOnly, InductOnly } tempinduct_cases = TempInductCases::Both;

		// Store the strings for signals that get set/proven. Once the module selection has been
		// determined, these are turned into sigspecs in the config
		StepSigVals<std::string> sets_str;
		StepSigs<std::string> unsets_str, sets_def_str, sets_any_undef_str, sets_all_undef_str;
		SigVals<std::string> sets_init_str, prove_str, prove_x_str;
		Sigs<std::string> shows_str;

		bool set_def_inputs = false;
		bool show_inputs = false, show_outputs = false;
		bool show_regs = false, show_public = false, show_all = false;
		bool prove_asserts = false;

		const auto require_and_set_mode = [&mode](const ProveMode new_mode) {
			if (mode != ProveMode::Comb && new_mode != mode)
				log_cmd_error("aaa\n");

			mode = new_mode;
		};

		const auto require_mode_in = [&mode](std::initializer_list<ProveMode> modes) {
			if (std::none_of(modes.begin(), modes.end(), [&mode](const auto new_mode) {return mode == new_mode;}))
				log_cmd_error("bbb\n");
		};
		...
		for (argidx = 1; argidx < args.size(); argidx++) {
			// General options
			if (args[argidx] == "-all") {
				require_mode_in({ProveMode::Comb, ProveMode::Seq});
				config.num_solutions = SatConfig::AllSolutions;
				continue;
			}
			if (args[argidx] == "-max" && argidx+1 < args.size()) {
				require_mode_in({ProveMode::Comb, ProveMode::Seq});
				config.num_solutions = atoi(args[++argidx].c_str());
				continue;
			}
			if (args[argidx] == "-enable_undef") {
				config.enable_undef = true;
				continue;
			}
			if (args[argidx] == "-max_undef") {
				config.enable_undef = true;
				config.max_undef = true;
				continue;
			}
			if (args[argidx] == "-set" && argidx+2 < args.size()) {
				const auto& lhs = args[++argidx];
				const auto& rhs = args[++argidx];
				sets_str[SatConfig::Always].emplace_back(lhs, rhs);
				continue;
			}
			if (args[argidx] == "-set-def" && argidx+1 < args.size()) {
				sets_def_str[SatConfig::Always].emplace(args[++argidx]);
				config.enable_undef = true;
				continue;
			}
			if (args[argidx] == "-set-any-undef" && argidx+1 < args.size()) {
				sets_any_undef_str[SatConfig::Always].emplace(args[++argidx]);
				config.enable_undef = true;
				continue;
			}
			if (args[argidx] == "-set-all-undef" && argidx+1 < args.size()) {
				sets_all_undef_str[SatConfig::Always].emplace(args[++argidx]);
				config.enable_undef = true;
				continue;
			}
			...
			if (args[argidx] == "-set-def-formal") {
				config.set_def_formal = true;
				config.enable_undef = true;
				continue;
			}
			if (args[argidx] == "-show" && argidx+1 < args.size()) {
				shows_str.emplace(args[++argidx]);
				continue;
			}
			...
			if (args[argidx] == "-ignore_div_by_zero") {
				config.ignore_div_by_zero = true;
				continue;
			}
			if (args[argidx] == "-ignore_unknown_cells") {
				config.ignore_unknown_cells = true;
				continue;
			}
			// Seq options (non-incremental BMC)
			if (args[argidx] == "-seq" && argidx+1 < args.size()) {
				require_and_set_mode(ProveMode::Seq);
				config.maxsteps = atoi(args[++argidx].c_str());
				continue;
			}
			if (args[argidx] == "-set-at" && argidx+3 < args.size()) {
				...
				const auto& lhs = args[++argidx];
				const auto& rhs = args[++argidx];
				sets_str[timestep].emplace_back(lhs, rhs);
				continue;
			}
			if (args[argidx] == "-unset-at" && argidx+2 < args.size()) {
				int timestep = atoi(args[++argidx].c_str());
				unsets_str[timestep].emplace(args[++argidx]);
				continue;
			}
			if (args[argidx] == "-set-assumes") {
				config.set_assumes = true;
				continue;
			}
			if (args[argidx] == "-set-def-at" && argidx+2 < args.size()) {
				int timestep = atoi(args[++argidx].c_str());
				sets_def_str[timestep].emplace(args[++argidx]);
				config.enable_undef = true;
				continue;
			}
			if (args[argidx] == "-set-any-undef-at" && argidx+2 < args.size()) {
				int timestep = atoi(args[++argidx].c_str());
				sets_any_undef_str[timestep].emplace(args[++argidx]);
				config.enable_undef = true;
				continue;
			}
			if (args[argidx] == "-set-all-undef-at" && argidx+2 < args.size()) {
				int timestep = atoi(args[++argidx].c_str());
				sets_all_undef_str[timestep].emplace(args[++argidx]);
				config.enable_undef = true;
				continue;
			}
			if (args[argidx] == "-set-init" && argidx+2 < args.size()) {
				const auto& lhs = args[++argidx];
				const auto& rhs = args[++argidx];
				sets_init_str.emplace_back(lhs, rhs);
				continue;
			}
			if (args[argidx] == "-set-init-undef") {
				config.set_init_undef = true;
				config.enable_undef = true;
				continue;
			}
			if (args[argidx] == "-set-init-def") {
				config.set_init_def = true;
				config.enable_undef = true;
				continue;
			}
			if (args[argidx] == "-set-init-zero") {
				config.set_init_zero = true;
				continue;
			}
			if (args[argidx] == "-dump_vcd" && argidx+1 < args.size()) {
				config.vcd_file_name = args[++argidx];
				continue;
			}
			if (args[argidx] == "-dump_json" && argidx+1 < args.size()) {
				config.json_file_name = args[++argidx];
				continue;
			}
			if (args[argidx] == "-dump_cnf" && argidx+1 < args.size()) {
				config.cnf_file_name = args[++argidx];
				continue;
			}
			// Temporal induction options
			if (args[argidx] == "-tempinduct") {
				require_and_set_mode(ProveMode::TempInduct);
				continue;
			}
			if (args[argidx] == "-tempinduct-def") {
				require_and_set_mode(ProveMode::TempInduct);
				config.tempinduct_def = true;
				continue;
			}
			if (args[argidx] == "-tempinduct-baseonly") {
				require_and_set_mode(ProveMode::TempInduct);
				tempinduct_cases = TempInductCases::BaseOnly;
				continue;
			}
			if (args[argidx] == "-tempinduct-inductonly") {
				require_and_set_mode(ProveMode::TempInduct);
				tempinduct_cases = TempInductCases::InductOnly;
				continue;
			}
			if (args[argidx] == "-tempinduct-skip" && argidx+1 < args.size()) {
				require_and_set_mode(ProveMode::TempInduct);
				config.tempinduct_skip = atoi(args[++argidx].c_str());
				continue;
			}
			if (args[argidx] == "-prove" && argidx+2 < args.size()) {
				const auto& lhs = args[++argidx];
				const auto& rhs = args[++argidx];
				prove_str.emplace_back(lhs, rhs);
				continue;
			}
			if (args[argidx] == "-prove-x" && argidx+2 < args.size()) {
				const auto& lhs = args[++argidx];
				const auto& rhs = args[++argidx];
				prove_x_str.emplace_back(lhs, rhs);
				continue;
			}
		...
		if (mode != ProveMode::TempInduct) {
			config.basecase = true;
			config.inductcase = false;
		} else {
			config.basecase   = tempinduct_cases != TempInductCases::InductOnly;
			config.inductcase = tempinduct_cases != TempInductCases::BaseOnly;
		}
		...
		if (mode == ProveMode::Comb)
			log_assert(config.maxsteps == 0);

		const auto parse_to_sigspec = [&](const std::string& s) -> RTLIL::SigSpec {
				RTLIL::SigSpec sigspec;
				if (!RTLIL::SigSpec::parse_sel(sigspec, design, module, s))
					log_cmd_error("Failed to parse expression `%s'.\n", s.c_str());
				return sigspec;
		};

		const auto parse_pair_to_sigspec = [&](const std::pair<std::string, std::string>& str) -> std::pair<RTLIL::SigSpec, RTLIL::SigSpec> {
			const auto lhs = parse_to_sigspec(str.first), rhs = parse_to_sigspec(str.second);
			if (lhs.size() != rhs.size())
				log_cmd_error("Set expression with different lhs and rhs sizes: %s (%s, %d bits) vs. %s (%s, %d bits)\n",
					str.first.c_str(), log_signal(lhs), lhs.size(), str.second.c_str(), log_signal(rhs), rhs.size());
			return {lhs, rhs};
		};

		const auto parse_stepsigs = [&](const StepSigs<std::string>& src, StepSigs<SigSpec>& target) {
			for (const auto& [t, strs] : src)
				for (const auto& str : strs)
					target[t].emplace(parse_to_sigspec(str));
		};

		// Convert set expressions to sigspecs
		for (const auto& [t, strs] : sets_str)
			for (const auto& p : strs)
				config.sets[t].emplace_back(parse_pair_to_sigspec(p));

		// Parser unsets/def/any_undef/all_undef
		parse_stepsigs(unsets_str, config.unsets);
		parse_stepsigs(sets_def_str, config.sets_def);
		parse_stepsigs(sets_any_undef_str, config.sets_any_undef);
		parse_stepsigs(sets_all_undef_str, config.sets_all_undef);

		// Convert set-init expressions to sigspecs
		for (const auto& p : sets_init_str)
			config.sets_init.emplace_back(parse_pair_to_sigspec(p));

		if (set_def_inputs) {
			for (const auto* wire : module->wires())
				if (wire->port_input)
					config.sets_def[SatConfig::Always].emplace(wire);
		}

		// Add port and public wire shows
		for (const auto* wire : module->wires())
			if (show_all                           ||
			   (show_inputs  && wire->port_input)  ||
			   (show_outputs && wire->port_output) ||
			   (show_public  && wire->name.isPublic()))
				config.shows.emplace(wire);

		// Add user specified show strings
		for (const auto& str : shows_str)
			config.shows.emplace(parse_to_sigspec(str));
			...
			for (const auto* cell : module->cells())
				if (RTLIL::builtin_ff_cell_types().count(cell->type))
					for (auto bit : cell->getPort(ID::Q))
						if (bit.wire)
							config.shows.emplace(bit.wire);
		}

		// Convert prove expressions to sigspecs
		for (const auto& p : prove_str)
			config.proof_goals.emplace_back(ProofGoal::prove_eq(parse_pair_to_sigspec(p)));

		// Convert prove-x expressions to sigspecs
		for (const auto& p : prove_x_str)
			config.proof_goals.emplace_back(ProofGoal::prove_eq_x(parse_pair_to_sigspec(p)));

		// Add selected $assert cells as proof goals
		if (prove_asserts) {
			for (const auto* cell : module->cells())
				if (cell->type == ID($assert) && design->selected(module, cell))
					config.proof_goals.emplace_back(cell);
		}

		if (mode == ProveMode::TempInduct && config.proof_goals.empty())
			log_cmd_error("Got -tempinduct but nothing to prove!\n");

		// if (mode == ProveMode::TempInduct && prove_skip)
